...
class UIManager:
    ...
    def run_interactive_session(self):
        ...
        try:
            ...
            print(f"❌ Unexpected Error during configuration validation: {str(e)}")
            sys.exit(1)
    ...
    def create_new_task(self) -> Optional[Dict]:
        ...
        print(f"Project: {self.config.jira_project_key}")
        if component_names:
    ...
    def format_task_output(task: Dict, jira_url: str) -> str:
        ...
        if task.get("_has_pr"):
            ...
            review_status = pr_manager.analyze_pr_reviews(reviews)
    ...
    @staticmethod
    def open_task_in_browser(task: Dict, jira_url: str):
        """Open a JIRA task in the default web browser."""
        key = task.get("key", "")
        if key:
            task_url = f"{jira_url.rstrip('/')}/browse/{key}"
            try:
    ...
    def _execute_task_action(self, action_type: str, task: Dict) -> None:
        ...
        if action_type == "browser":
            self.open_task_in_browser(task, self.config.jira_url)
            ...
            try:
                pr_url = self.pr_manager.get_existing_pr_url(task)
                print(f"✅ Opening PR for {task_key} in browser...")
                webbrowser.open(pr_url)
                print(f"\n🎉 PR opened for {task_key}!")
            except PRManagerException as e:
                print(f"❌ Failed to open PR: {str(e)}")
            self.wait_for_continue()
    ...
    def display_interactive_tasks(self, issues: List[Dict]):
        ...
        while True:
            ...
            if not issues:
                ...
                print(
                    "Use ↑/↓ arrow keys to navigate, Enter for actions, 'n' to create new task, 'r' to refresh, 'q'/ESC to quit"
                )
                self.print_header("", 80)

                # Display tasks with selection indicator
                formatted_tasks = [
                    self.format_task_output(task, self.config.jira_url)
                    for task in issues
                ]
                ...
                self.print_header("", 80)
                print(
                    f"Selected: {selected_index + 1}/{len(issues)} | Press 'h' for help, 'r' to refresh"
                )
